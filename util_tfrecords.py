...
def combine_tfrecords(
    ...
    writer = tf.data.experimental.TFRecordWriter(
        fn_dst, compression_type=compression_type
    )
    if verbose:
...
def get_dataset_from_tfrecords(
    ...
    if feature_description is None:
        ...
        feature_description = get_feature_description_from_tfrecord(
            filenames, compression_type=compression_type
        )
    ...
    ), "invalid feature_description specified"
    ...
    def _parse_function(example_proto):
        ...
        if feature_description:
            example_parsed = tf.io.parse_single_example(
    ...
    def _densify_function(example):
        ...
        for k_dense_shape in list_k_dense_shape:
            ...
            if densify_downsample_factors is not None:
                dense_shape = tf.math.floordiv(dense_shape, densify_downsample_factors)
            ...
            for itr_dim in range(len(dense_shape)):
                ...
                if densify_downsample_factors is not None:
                    indices_dim = tf.math.floordiv(
                        indices_dim, densify_downsample_factors[itr_dim]
                    )
                if densify_jitter_indices is not None:
                    if densify_jitter_indices[itr_dim] > 0:
                        ...
                        indices_dim = tf.clip_by_value(
                            indices_dim, 0, dense_shape[itr_dim] - 1
                        )
            ...
            if (densify_downsample_factors is not None) or (
                densify_jitter_indices is not None
            ):
                uniqe_indices, map_to_unique_indices = tf.raw_ops.UniqueV2(
                    x=indices, axis=[0], out_idx=tf.int64
                )
                uniqe_indices = tf.ensure_shape(
                    uniqe_indices, [None, indices.shape[-1]]
                )
                map_to_unique_indices = tf.reshape(map_to_unique_indices, [-1])
                values = tf.math.unsorted_segment_sum(
                    values, map_to_unique_indices, tf.shape(uniqe_indices)[0]
                )
                indices = uniqe_indices
            example_densified[k] = tf.sparse.to_dense(
                tf.sparse.reorder(tf.sparse.SparseTensor(indices, values, dense_shape))
            )
        for k in example:
